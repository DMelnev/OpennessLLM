...
## 29. Кириллица бьётся (mojibake) внутри самого блока после apply-clone

Отличать от раздела 25 (там ломается только отображение в PowerShell, файл
цел). Здесь ломается сам файл, экспортированный обратно из TIA после
`apply-clone`/`compile-block` - REGION-заголовки и `//`-комментарии превращаются
в классический double-UTF-8 mojibake (`Ð—Ð°ÐºÐ°Ð·Ñ‹` вместо `Заказы`), причём
испорченным оказывается весь файл целиком, а не только строка с REGION.

Симптомы:

```text
после apply-clone -> compile-block -> check-clone свежий экспорт блока
(CLONE_PROJECT\_compare\current-*\...) читается как UTF-8, но кириллица - мусор;
блок был полностью пересоздан внешним источником (не точечная правка);
в теле блока есть REGION;
исходный clone-файл на диске (тот, что отправляли в TIA) при этом в порядке -
проверяется побайтово, BOM и UTF-8 корректны.
```

Проверено (2026-09-25, TIA Portal V18, ET 200SP F-CPU, проект на кириллице):

```text
REGION, созданный вручную в TIA Portal (через UI, не через apply-clone) -
всегда цел, с любым именем на кириллице;
REGION в файле, отправленном через apply-clone как ТОЧЕЧНАЯ правка
(Edit/патч нескольких строк существующего файла) - тоже остаётся цел, включая
уже существующие REGION с кириллическими названиями;
REGION в файле, который LLM создал ЗАНОВО целиком (полная перезапись .scl
для FB/FC/OB) и затем отправил через apply-clone - ломается целиком, включая
обычные // комментарии вне REGION. Смена языка REGION-заголовка на латиницу
проблему не решает - ломается всё равно весь файл.
```

Где смотрели причину и что нашли:

```text
GenerateBlockFromCloneSource -> CreateExternalSourceFromFile (Program.cs) не
читает и не перекодирует содержимое файла - передаёт Siemens API только путь
(Path.GetFullPath(sourcePath)) через ExternalSources.CreateFromFile(name, path),
дальше GenerateBlocksFromSource() - тоже голый вызов Openness API. OpennessLLM
байты файла нигде не трогает. Значит разбор (включая кодировку) идёт уже
внутри закрытой сборки Siemens.Engineering - воспроизвести/исправить в этом
репозитории нечем, это не баг в Program.cs (либо баг в Openness API, либо
не до конца понятая чувствительность к состоянию файла/содержимому в момент
CreateFromFile - не подтверждено).
```

Что делать (практический обход, 100% против 0% на выборке этой сессии):

```text
1. Для существующего FB/FC/OB с REGION и кириллицей - не переписывать файл
   целиком. Вносить изменения точечно (Edit/патч конкретных строк), даже
   если правка большая по объёму.
2. Если блок объективно нужно пересобрать с нуля (новая структура, много
   правок) - после apply-clone -> compile-block сразу проверить свежий
   экспорт (check-clone -> CLONE_PROJECT\_compare\current-*\...) на
   побайтовую целостность кириллицы, ДО save. Пример проверки:
```

```powershell
python -c "print(open(r'CLONE_PROJECT\_compare\current-*\_root\<file>.scl', encoding='utf-8-sig').read()[:500])"
```

```text
3. Если побилось - НЕ сохранять проект. Откатить (закрыть без сохранения
   либо восстановить из backup в TIA_PROJECT_BACKUPS, который apply/compile
   создают перед каждой записью) и переделать блок через точечные правки.
4. Сохранение проекта (--save) - явное решение владельца проекта, не делать
   автоматически поверх неподтверждённого результата apply/compile.
```
